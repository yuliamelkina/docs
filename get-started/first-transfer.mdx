...
- Consider minimum and maximum transfer limits

### Customer Verification
- Always verify customer status is `active` before creating transfers
- Check that required endorsements are `approved`
- Validate external account ownership

### Error Recovery
- Implement retry logic for transient failures
- Handle state transitions gracefully
- Provide clear user messaging for different error conditions

### Compliance
- Store transfer purposes and references for audit trails
- Implement appropriate KYC/AML checks based on amounts
- Consider regulatory requirements for different payment rails
